...
def _maybe_auto_refresh(client: httpx.Client) -> bool:
    """Try to refresh cookies using ~/.openclaw/.env and reset client's cookie jar."""
    load_dotenv()
    try:
        from login_refresh import main as refresh_main  # type: ignore

        refresh_main()
        # reload cookie jar from state file
        client.cookies = cookies_from_state()
        client.get(INDEX)
        return True
    except Exception:
        return False
...
def fetch_offerings(client: httpx.Client, *, term: str, code: str) -> List[Dict[str, str]]:
    ...
    Auto behavior:
    - If session is expired, we attempt a one-shot cookie refresh (requires HANSUNG_INFO_ID/PASSWORD in ~/.openclaw/.env)
      then retry once.
    """

    xml = client.post(HISTORY, data={"gubun": "history", "syearhakgi": term, "sjungong": code}).text
    if "로그인 정보를 잃었습니다" in xml:
        if _maybe_auto_refresh(client):
            xml = client.post(HISTORY, data={"gubun": "history", "syearhakgi": term, "sjungong": code}).text
